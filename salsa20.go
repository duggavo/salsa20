...
import (
	"github.com/duggavo/salsa20/salsa"
)
...
func XORKeyStream(out, in []byte, nonce []byte, key *[32]byte, iterations int) {
	if len(out) < len(in) {
	...
	if len(nonce) == 24 {
		...
		salsa.HSalsa20(&subKey, &hNonce, key, &salsa.Sigma, iterations)
